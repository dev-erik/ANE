...
def main():
    ...
    if zip_size > MAX_ZIP_SIZE:
        print(f"ERROR: ZIP file too large ({zip_size/1e9:.1f} GB > {MAX_ZIP_SIZE/1e9:.0f} GB limit).",
              file=sys.stderr)
        print(f"  Set MAX_ZIP_BYTES=<bytes> to adjust the limit.", file=sys.stderr)
        sys.exit(1)
    ...
    with zipfile.ZipFile(ZIP_PATH, 'r') as z:
        if 'data00.bin' not in z.namelist():
            print(f"ERROR: 'data00.bin' not found in ZIP.", file=sys.stderr)
            print(f"  ZIP contents: {z.namelist()}", file=sys.stderr)
            sys.exit(1)
    ...
    with open(OUTPUT_PATH, 'rb') as f:
        raw = f.read(20)
        if len(raw) < 20:
            print(f"WARNING: Output file very small ({len(raw)} bytes) — possibly empty.",
                  file=sys.stderr)
        else:
            tokens = struct.unpack('<10H', raw)
            invalid = [t for t in tokens if t >= VOCAB_SIZE]
            if invalid:
                print(f"WARNING: Invalid token IDs in first 10 tokens: {invalid} "
                      f"(expected < {VOCAB_SIZE})", file=sys.stderr)
            else:
                print(f"First 10 tokens: {tokens} (all < {VOCAB_SIZE} OK)")
